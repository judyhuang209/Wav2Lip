...
class Wav2Lip(nn.Module):
    def __init__(self):
        ...
        self.face_encoder_blocks = nn.ModuleList([
            nn.Sequential(cbam_Conv2d(6, 16, kernel_size=7, stride=1, padding=3)), # 96,96
    ...
    # concatenate audio and face sequences
    def forward(self, audio_face_sequences):
        # audio_sequences = (B, T, 1, 80, 16)
        # face_sequences = (B, 6, T, 96, 96)
        # split(audio, face)
        # view(audio), view(face)
        audio_sequences, face_sequences = torch.split(audio_face_sequences, [syncnet_T*80*16, 6*syncnet_T*hparams.img_size*hparams.img_size], dim=-1)
        audio_sequences =  audio_sequences.view(audio_face_sequences.size(0), syncnet_T, 1, 80, 16)
        face_sequences =  face_sequences.view(audio_face_sequences.size(0), 6, syncnet_T, hparams.img_size, hparams.img_size)
        print('face_sequences', face_sequences.shape)
        print('audio_sequences', audio_sequences.shape)
        ...
        # print('face seq shape', face_sequences.shape) # B*5

        feats = []
        x = face_sequences
        # print('face encoder size:')
        ...
        for f in self.face_decoder_blocks:
            ...
            # print(x.shape)
            feats.pop()
        print('before output_block size', x.shape)
        ...
        print('model output len', len(outputs))
...
class Wav2Lip_disc_qual(nn.Module):
    ...
    def perceptual_forward(self, false_face_sequences, gt_face_sequences):
        ...
        bce_loss = F.binary_cross_entropy(self.binary_pred(false_feats).view(len(false_feats), -1), 
                                        torch.ones((len(false_feats), 1)).cuda())
        
        # code: https://github.com/VainF/pytorch-msssim
        # paper: https://ieeexplore.ieee.org/document/1284395
        false_pred_loss = 1 - ssim( false_face_sequences, gt_face_sequences, data_range=1, size_average=True ) + bce_loss
